const { test, expect, beforeEach } = require("@playwright/test");

beforeEach("Before Each", async ({ page }) => {
  await page.waitForTimeout(1000);
});
test.describe("All HTML Assignments at once @htmlAssignments", async () => {
  // Assignment 1: Textbox
  test("Textbox Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/textbox.html");
    await page.fill('input[type="text"]', "John Doe");
  });

  // Assignment 2: Checkbox
  test("Checkbox Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/checkbox.html");
    await page.check("input#checkbox1");
    await page.check("input#checkbox2");
  });

  // Assignment 3: Radio Button
  test("Radio Button Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/radio.html");
    await page.click("input#radio1");
  });

  // Assignment 4: Dropdown
  test("Dropdown Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/dropdown.html");
    await page.selectOption("select#dropdown", "Canada");
  });

  // Assignment 5: Button
  test("Button Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/submit.html");
    await page.click("button#submitButton");
  });

  // Assignment 6: Password Input
  test("Password Input Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/password.html");
    await page.fill('input[type="password"]', "secretpassword");
  });

  // Assignment 7: Textarea
  test("Textarea Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/textarea.html");
    await page.fill("textarea", "This is a sample message.");
  });

  // Assignment 8: Multiple Select
  test("Multiple Select Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/multiSelect.html");
    await page.selectOption("select#colors", ["red", "blue"]);
  });

  // Assignment 9: Headings
  test("Headings Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/heading.html");
    const headingText = await page.evaluate(() => {
      return {
        heading1: document.querySelector("h1").innerText,
        heading2: document.querySelector("h2").innerText,
        heading3: document.querySelector("h3").innerText,
      };
    });
    console.log("Headings:", headingText);
  });

  // Assignment 10: File Upload
  test("File Upload Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/fileUpload.html");
    const filePath = "D:/file_list.txt"; // Specify the path to the file to upload
    const fileInput = await page.locator('input[type="file"]');
    await fileInput.setInputFiles(filePath);
  });

  // Assignment 11: Drag and Drop
  test("Drag and Drop Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/dragAndDrop.html");
    const dragElement = await page.locator("#dragElement");
    const dropZone = await page.locator("#dropZone");
    await dragElement.dragTo(dropZone);
  });

  // Assignment 12: Image Tag
  test("Image Tag Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/image.html");
    const imageUrl = await page.evaluate(() => {
      const imgElement = document.querySelector("img");
      imgElement.src =
        "data:image/jpeg;base64,";

      return imgElement ? imgElement.src : null;
    });
    console.log("Image URL:", imageUrl);
  });

  test("Handle Dialog Event page.on ", async ({ page }) => {
    // Open a sample page that triggers a dialog
    await page.goto("http://localhost:9090/assignments/confirm.html");

    // Listen for 'dialog' events using page.on
    page.on("dialog", async (dialog) => {
      console.log("Dialog Type:", dialog.type());
      console.log("Dialog Message:", dialog.message());

      // Example: Handle different types of dialogs
      if (dialog.type() === "alert") {
        console.log("Handling Alert Dialog");
        await dialog.accept(); // Accept the alert dialog
      } else if (dialog.type() === "confirm") {
        console.log("Handling Confirm Dialog");
        await dialog.accept(); // Accept the confirm dialog
      } else if (dialog.type() === "prompt") {
        console.log("Handling Prompt Dialog");
        await dialog.accept("Playwright"); // Accept the prompt dialog with a custom value
      }
    });

    // Click a button or perform an action that triggers a dialog
    await page.evaluate(() => {
      alert("This is an alert dialog!");
    });

    const button = await page.locator("button[onclick='showAlert()']");
    await button.click();

    const showPromptButton = await page.locator(
      'button:has-text("Show Prompt")'
    );
    await showPromptButton.click();

    // Wait for the dialog event to be handled
    // Note: You may not need to wait explicitly with page.on
  });

  //confirm dialog box

  //new page and new window
  test("New Page and New Window Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/newPage.html");

    // Test New Page
    const newPageLink = await page.locator("a");
    await newPageLink.click();
    await page.waitForTimeout(1000); // Wait for new page to load

    // Test New Window
    const newWindowButton = await page.locator(
      'button:has-text("Open New Window")'
    );
    await newWindowButton.click();
    const newWindow = await page.waitForEvent("popup");
    await newWindow.waitForLoadState();
    console.log("New Window URL:", newWindow.url());
  });

  //frames assignment
  test("Frames (IFrames) Assignment", async ({ page }) => {
    await page.goto("http://localhost:9090/assignments/frames.html");

    // Test Frame 1
    const frame1 = await page.$("#frame1Id");
    const frame1Content = await frame1.contentFrame();
    console.log("Frame 1 Title:", await frame1Content.title());
    console.log("Frame 1 Name:", await frame1Content.name());
    console.log("Frame 1 content:", await frame1Content.content()); // console.log("Frame 1 Content:", title);

    // Test Frame 2
    const frame2 = await page.$("#frame2Id");

    const frame2Title = await frame2.contentFrame();
    console.log("Frame 2 Title:", await frame2Title.title());
    console.log("Frame 2 Name:", await frame2Title.name());
    console.log("Frame 2 content:", await frame2Title.content()); // console.log("Frame 1 Content:", title);
  });
});
